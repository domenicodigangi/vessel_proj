...
def encode_features(
    data,
    cols_to_drop=["PORT_NAME", "REGION_NO"],
...
def impute_missing(data, imputer_missing):
    ...
    X.loc[:] = my_imputer.fit_transform(X)
...
@task
def train_score_model(train_X_y, test_X_y, model_name, cv_n_folds):
...
@task
def estimate_sage(train_X_y, test_X_y, model_name, sage_imputer, n_sage_perm):
...
def one_run(
    ...
    with wandb.init(
        ...
        train_score_model(train_X_y, test_X_y, model_name, cv_n_folds)

        if run_sage in ["True", "Y", "T", True]:
            estimate_sage(train_X_y, test_X_y, model_name, sage_imputer, n_sage_perm)
