...
def get_list_df_traj_from_df_full_traj(df_full_traj: pd.DataFrame):
    ...
    df_full_traj["traj_group"] = (df_full_traj["datetime"].diff() > max_delta).cumsum()
    df_full_traj["traj_group"].iloc[0] = df_full_traj["traj_group"].iloc[1]
    df_full_traj["lat_long"] = df_full_traj[["latitude", "longitude"]].apply(
        tuple, axis=1
    )

    df_full_traj = df_full_traj.drop(df_full_traj[df_full_traj["latitude"] > 90].index)
    df_full_traj = df_full_traj.drop(df_full_traj[df_full_traj["latitude"] < -90].index)
...
for traj_id, label_filepath in filepath_labels_dict.items():
    ...
    for i, filepath in enumerate(traj_subfold.iterdir()):
        ...
        ind_same_start = df_full_traj["datetime"].iloc[0] == df_labels["Start Time"]
        ind_same_end = df_full_traj["datetime"].iloc[-1] == df_labels["End Time"]
