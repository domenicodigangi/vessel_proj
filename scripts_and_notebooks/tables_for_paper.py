...
for col in ["avg_centr", "avg_rank_centr"]:
    df_tab = (
        df.merge(df_feat, left_index=True, right_index=True)
        .sort_values(by=col, ascending=False)[["PORT_NAME", col]]
        .head(10)
        .reset_index()
        .drop(columns=["index"])
    )
...
def make_and_save_latex_tab_sage_imp(df, tab_name, col_names):
    ...
    features = [
        c.replace("sage_mean_", "") for c in df.columns if c.startswith("sage_mean")
    ]
    df_mean = df[
        ["disc_strategy"] + [c for c in df.columns if c.startswith("sage_mean")]
    ].rename(columns={f"sage_mean_{n}": n for n in features})
