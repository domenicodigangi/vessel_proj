...
def clean_group_edges(df_edges):
    df_edges = set_types_edge_list(df_edges)

    df_edges_clean = clean_edges(df_edges)

    dfg_edges = group_edges(df_edges_clean)
...
def group_edges(df_edges: pd.DataFrame) -> pd.DataFrame:
    #%% group trips
    ...
    df_edges_grouped = df_edges.groupby(["start_port", "end_port"], observed=True).agg(
        duration_avg_days=("duration_days", np.mean),
        trips_count=("uid", count_unique),
        vesseltype_count=("vesseltype", count_unique),
    )
